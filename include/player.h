...
typedef struct
{
    ...
    Texture2D hurt_texture;             // Character texture when hurt by monster
    ...
    int max_hearts;
    bool is_dead;
    bool is_hurt;             // True when player has been hit by monster
    float hurt_timer;         // Timer for hurt effect duration
    bool is_burnt;            // True when player has hit lava
    float burnt_timer;        // Timer for burnt effect duration
